      ...
      if(this.address=="not_select"){
        this.apiserv.updatedeliveryslots(this.sid, this.date, this.slot).subscribe(data => {
          ...
          if (response['status'] == 'Valid') {
            ...
            setTimeout(() => {
              this.route.navigate(['deliveryaddress', { totalamount: this.totalamount,date:this.date,time:this.slot,walletCheck:this.wallet_check,sample_collection_type:this.sample_collection_type,prescription:this.prescription}]);
