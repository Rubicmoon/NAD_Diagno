...
export class DeliveryaddressPage implements OnInit {
  ...
  constructor(private apiserv: ApiService,
    ...
    this.sample_collection_type = this.actroute.snapshot.paramMap.get('sample_collection_type');
    this.prescription = this.actroute.snapshot.paramMap.get('prescription');
  }
  ...
  addAddress(){
      this.route.navigate(['addaddress',{walletCheck:this.wallet_check,sample_collection_type:this.sample_collection_type,prescription:this.prescription}]);
  ...
  Delivery_address() {
    ...
    this.apiserv.deliverytothisaddress(this.user_id, this.sid, this.selected_address_id, this.cityId,this.sample_collection_type,this.prescription).subscribe(data => {
      var response = data['response'][0];
      this.utilserv.dismissLoading();
      if (response['status'] == 'Valid') {
