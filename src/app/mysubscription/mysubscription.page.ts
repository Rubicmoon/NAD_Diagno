  ...
  async doPaytm(amount){
    let that = this;
     let loading = await this.loadingCtrl.create({
      ...
      loading.present();
    var inappbrowser = that.iab.create("https://nadhospital.com/app/webservices/paytm-buy-subscription?plan_id=" + 
    that.package_id + "&user_id=" + that.user_id, '_blank', { location: 'no', zoom: 'yes', hideurlbar: 'yes' });
    var success_url = 'https://nadhospital.com/app/webservices/paytm-bs-response';
    var failure_url = '';
    // alert(inappbrowser);
