...
export class AppComponent {
  ...
  checkRoute(){
    this.apiserv.getSettings().subscribe(data => {
      ...
      if (response['status'] == 'Valid') {
        this.utilserv.setting_data = response;
        if(this.utilserv.setting_data.google_map_location=='Yes'){
        myVar = document.createElement('script');
        var key=this.utilserv.setting_data.google_map_api_key;
        myVar.setAttribute('src','https://maps.google.com/maps/api/js?key='+key+'&amp;libraries=places,geometry');
        document.head.appendChild(myVar);
      }
        console.log(this.utilserv.setting_data);
        localStorage.setItem('multi_language',this.utilserv.setting_data.multi_language);
        this.language=localStorage.getItem('multi_language');
  ...
  initializeApp() {
    ...
    this.platform.ready().then(() => {
      ...
      this.renderer.setAttribute(document.body, 'color-theme', 'light');
      this.statusBar.backgroundColorByHexString('#253ba0');
