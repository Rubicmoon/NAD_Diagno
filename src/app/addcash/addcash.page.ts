  ...
  async doPaytm(amount){
    let that = this;
     let loading = await this.loadingCtrl.create({
      ...
      loading.present();
    var inappbrowser = that.iab.create("https://nadhospital.com/app/webservices/paytm-initiate-add-cash?amount=" + 
    amount + "&user_id=" + that.user_id, '_blank', { location: 'no', zoom: 'yes', hideurlbar: 'yes' });
    var success_url = 'https://nadhospital.com/app/webservices/paytm-ac-response';
    var failure_url = '';
    // alert(inappbrowser);
