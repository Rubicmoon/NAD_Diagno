...
@Component({
  selector: 'app-specialtag',
  templateUrl: './specialtag.page.html',
  styleUrls: ['./specialtag.page.scss'],
})
export class SpecialtagPage implements OnInit {
  apirequest:any;
  ...
  async filtermodal() {
    ...
    modal.onDidDismiss().then(function (res) {
      ...
      if(res.role=="backdrop"){
        // modal.dismiss();
        that.router.navigate(['/specialtag',{tag: this.tag}]);
      }
      else{
        var myArray=[];
  ...
  async getProductList() {
    ...
    this.start = 0;
    this.limit = 10;
    this.apiserv.getproductsByTag(this.sid, this.city_id, this.start, this.limit,this.tag,this.sort_type,language_id,this.user_id,this.brand).subscribe(data => {
        ...
        if (response['status'] == 'Valid') {
          ...
          if(response['tag']){
            this.special_tag=response['tag'];
          }
          else{
            this.special_tag="Special Offers";
          }
  ...
  increase(cart_id, addid,product) {
    ...
    for(var i=0;i<product.price.length;i++){
      if(product.selectedItem==product.price[i].id){
        ...
        // if(product.price[i].cart_qty<product.cart_limit){
        //   product.price[i].cart_qty++;
        //   select_quantity=product.price[i].cart_qty;
        //   this.add_id = addid;
        //   this.products.forEach(ob => {
