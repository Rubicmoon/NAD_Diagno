...
export class AddaddressPage implements OnInit {
  ...
  async addadressSubmit(addadressForm) {
    ...
    if (this.cityId == "" || this.cityId == undefined) {
      this.utilserv.presentAlert('Select City');
      return;
    }
    ...
    this.apiserv.addaddressform(
      ...
      if (response['status'] == 'Valid') {
        ...
        setTimeout(() => {
          ...
          that.route.navigate(['deliveryaddress', { totalamount: total,walletCheck: that.wallet_check,sample_collection_type:this.sample_collection_type,prescription:this.prescription}]);
