...
export class HomePage {
  ...
  updateapp() {
    window.open("https://play.google.com/store/apps/details?id=com.ndk_mart", "_system");
  }
  ...
  hotlinesales() {
    ...
    var tag = 'Top Medicine';
    var start = 0;
    var limit = 10;
    this.apiserv.hotline(tag, start, limit, this.city_id,language_id,this.sid).subscribe(data => {
      var response = data['response'][0];
  ...
  bestSellerProductList() {
    ...
    this.apiserv.hotline(tag, start, limit, this.city_id,language_id,this.sid).subscribe(data => {
      var response = data['response'][0];
      if(response.total_products!=0){
        this.bestsellProducts = response['products'];
        this.bestsellProducts.forEach(element => {
          element['quantity'] = 0;
          console.log(element);
          if(element.other_language_name){
            this.other_class=true;
        ...
        console.log(this.bestsellProducts);
      }
      else{
  ...
  addtocartitemBest(product) {
    ...
    if (this.user_id == '' || this.user_id == undefined || this.user_id == null) {
      ...
      this.selectIddBest=product.price[0].id;
      product.price[0].cart_qty++;
      this.apiserv.addtocart(this.product_id, product.price[0].id, this.sid, product.price[0].cart_qty,this.user_id).subscribe(data => {
        console.log(data);
        var response = data['response'][0];
  ...
  increaseBest(cart_id, addid,selectIdd,item) {
    alert("coming");
    this.add_idBest = addid;
    this.bestsellProducts.forEach(ob => {
      if (ob.id == cart_id) {
        ob.price[0].cart_qty = parseInt(ob.price[0].cart_qty) + 1;
        this.updatecartitemBest(ob.id, ob.price[0].cart_qty,selectIdd);
        this.cartcount();
  ...
  decreaseBest(cart_id, addid,selectIdd,item) {
    
    this.add_idBest = addid;
    this.bestsellProducts.forEach((ob, index, object) => {
      if (ob.id == cart_id) {
        if (ob.price[0].cart_qty >= 1) {
          ob.price[0].cart_qty--;
          // this.addtocartitem(ob.quantity);
  ...
  updatecartitemBest(cart_id, qty,selectIdd) {
    // alert("update");
    this.apiserv.addtocart(cart_id, selectIdd, this.sid, qty,this.user_id).subscribe(data => {
      console.log(data);
      var response = data['response'][0];
