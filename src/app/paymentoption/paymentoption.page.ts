  ...
  async paytmPay(){
    let that = this;
     let loading = await this.loadingCtrl.create({
      ...
      loading.present();
    var inappbrowser = that.iab.create("https://nadhospital.com/app/webservices/initiate-with-paytm?sid=" + 
    that.sid + "&user_id=" + that.user_id+"&payment_option=" +that.payment_type, '_blank', { location: 'no', zoom: 'yes', hideurlbar: 'yes' });
    var success_url = 'https://nadhospital.com/app/webservices/paytm-response';
    var failure_url = '';
    // alert(inappbrowser);
